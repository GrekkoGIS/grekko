...
use warp::Filter;
use vrp_pragmatic::format::problem::{Problem, PragmaticProblem};
...
pub async fn simple_trip(trip: request::SimpleTrip) -> Result<impl warp::Reply, Infallible> {
    ...
    let problem = Arc::new(core_problem.expect("Could not read a pragmatic problem into a core problem"));
